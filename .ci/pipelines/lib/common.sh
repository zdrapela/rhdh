...
common::oc_login() {
  ...
  log::info "Logging into OpenShift cluster at ${K8S_CLUSTER_URL}..."
  ...
  if ! oc whoami &> /dev/null; then
    log::error "Authentication verification failed for OpenShift cluster at ${K8S_CLUSTER_URL}"
    return 1
  fi
...
common::kubectl_login() {
  ...
  log::info "Logging into Kubernetes cluster at ${K8S_CLUSTER_URL}..."
  if ! kubectl config set-credentials sa-user --token="${K8S_CLUSTER_TOKEN}" &> /dev/null \
    ...
    log::error "Failed to configure kubectl for Kubernetes cluster at ${K8S_CLUSTER_URL}"
    return 1
  fi

  if ! kubectl auth can-i get nodes &> /dev/null; then
    log::error "Authentication verification failed for Kubernetes cluster at ${K8S_CLUSTER_URL}"
    return 1
  fi
